(function( angular ) {
  /*
   * Objectives
   * * Build a DAO which wraps around data access
   * * Move appropriate code out of payee-main.component.js and into the new DAO
   *
   * Note: From now until the completion of the Ajax chapter, the route
   * http://localhost:8000/banking/#!/tx/edit/{id} will not work
   *
   * Create a DAO as a factory. It will be a member of the payee module.
   * It should be called payeeDAO
   * It should return an object that has the following methods:
   *
   * get: Takes an id as an argument, returns a single matching Payee
   * query: Takes a criteria object as an argument, returns an array
   *        of matching Payees
   *
  ...
  angular.module( 'payee' )
    .factory( 'payeeDAO', function( $filter ) {
      ...
      function get( id ) {
        return filter( payees, { id: Number( id ) } )[ 0 ];
      }

      function query( criteria ) {
        return filter( payees, criteria );
      }
    } );
