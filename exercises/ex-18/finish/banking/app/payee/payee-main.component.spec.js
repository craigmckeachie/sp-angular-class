...
describe( 'Testing payeeMain component', function() {
  var $componentController, $q, $rootScope, payeeDAO, mockState, mockStateParams;
  ...
  beforeEach( function() {
    ...
    module( function( $provide ) {
      $provide.factory( 'payeeDAO', function() {
        return {
          get  : function() {
            return $q.resolve( getPayees()[ 0 ] );
          },
          query: function() {
            return $q.resolve( getPayees() );
          }
        }
      } )
    } );

    inject( function( _$componentController_, _$q_, _$rootScope_, _payeeDAO_ ) {
    ...
    mockState = { $current: { name: 'payees.detail' } };
    mockStateParams = { id: 47 };
  } );

  it( 'should load the payeeMain component', function() {
    var ctrl = $componentController( 'payeeMain',
      {
        $log        : console,
        $state      : mockState,
  ...
  it( 'should load a payee for payeeDetail when missing a payee', function() {
    var ctrl = $componentController( 'payeeMain',
      {
        $log        : console,
        $state      : mockState,
    ...
    spyOn( payeeDAO, 'get' ).and.callThrough();

    expect( ctrl.$onInit ).not.toHaveBeenCalled();
    expect( payeeDAO.get ).not.toHaveBeenCalled();
    expect( ctrl ).toBeDefined();
    ...
    expect( payeeDAO.get ).toHaveBeenCalled();

  } );

  it( 'should load payees for payeeList when missing payees', function() {
    mockState = { $current: { name: 'payees.list' } };
    ...
    spyOn( ctrl, '$onInit' ).and.callThrough();
    spyOn( payeeDAO, 'query' ).and.callThrough();

    expect( ctrl.$onInit ).not.toHaveBeenCalled();
    expect( payeeDAO.query ).not.toHaveBeenCalled();
    expect( ctrl ).toBeDefined();
    ...
    expect( ctrl.$onInit ).toHaveBeenCalled();
    expect( payeeDAO.query ).toHaveBeenCalled();
