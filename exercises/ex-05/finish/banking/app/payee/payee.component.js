(function( angular ) {
  angular.module( 'payee.component', [ 'payee.list', 'payee.detail', 'payee.search', 'ui.router' ] )
      .component( 'payeeComponent', {
        templateUrl: 'app/payee/payee-component-tpl.html',
        controller: PayeeComponentController
      } );

  function PayeeComponentController( $log, $state, $filter ) {
    var ctrl = this;

    ctrl.search = search;
    ctrl.getDetail = getDetail;

    function search( criteria, go ) {
      ctrl.payees = $filter( 'filter' )( getPayees(), criteria );
      if ( go )
        $state.go( 'payees.list', criteria );
    }

    function getDetail( passedPayee, go ) {
      if ( typeof passedPayee === 'object' ) {
        ctrl.payee = passedPayee;
      } else if ( typeof passedPayee === 'string' ) {
        getPayees().some( function( payee ) {
          if ( payee.payeeId = passedPayee ) {
            ctrl.payee = payee;
            return true;
          }
        } );
      } else {
        $log.error( 'PayeeComponentController.getDetail(): That\'s not a payee or a payeeId: [%s]', passedPayee );
      }
