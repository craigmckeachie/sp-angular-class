(function( angular ) {
  angular.module( 'componentRequire', [] )
    .component( 'levelOne', {
      ...
      controller: function() {
        var ctrl = this;
        ctrl.give = function( val ) {
          ctrl.level2Value = val;
        };
        ctrl.take = function() {
          return ctrl.level1Value;
        };
        ctrl.passedMessage = 'This is a message for level 2.';
      }
    } )
    .component( 'levelTwo', {
      template  : '<div style="padding:20px">' +
      ...
      '<p>$ctrl.level2Value is {{ $ctrl.level2Value }}.</p>' +
      '<p>$ctrl.parent.passedMessage is {{ $ctrl.parent.passedMessage }}</p>' +
      '</div>',
      require   : {
        parent: '^levelOne'
      },
      controller: function() {
        var ctrl = this;
        console.log( 'At creation: levelTwo.ctrl.parent: ', ctrl.parent );

        ctrl.$onInit = function() {
          console.log( '$onInit: levelTwo.ctrl.parent: ', ctrl.parent );
        };

        ctrl.updateParent = function( val ) {
          ctrl.parent.give( val );
        };
      }
    } )
})( angular );